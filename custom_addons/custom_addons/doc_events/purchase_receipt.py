...
def validate(self, method):
    ...
    purchase_receipt_total = sum(
        flt(row.grand_total) for row in purchase_receipts
    )
    ...
    expense_claim_total = sum(
        flt(row.grand_total) for row in expense_claims
    )
    ...
    if total >= limit:
        frappe.msgprint(
            f"Validation Error: The total amount for Petty Cash Request "
            f"'{self.petty_cash_request}' has exceeded the limit of {limit}. "
